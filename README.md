# I'm Samia Zahid
Master's Student in Computer Science at Soonchunhyang University, South Korea.
My research focuses on Artificial Intelligence, Machine Learning, and Computer Vision, with an interest in developing intelligent, scalable, and trustworthy AI systems for real-world applications.

## Contact
Email: samiazahid@gmail.com
LinkedIn:https://linkedin.com/in/yourprofile

## Research Interests
- Artificial Intelligence
- Machine Learning
- Computer Vision
- Deep Learning
- Intelligent Systems
- Explainable AI

- ## Technical Skills
- Python
- Java
- C++
- SQL
- MATLAB

## AI Frameworks
- PyTorch
- TensorFlow
- Keras
- YOLO

## Tools
- Git
- Linux
- LaTeX
- Overleaf

## Current Research
- Lightweight Deepfake Face Detection
- Industrial IoT Anomaly Detection
- Deep Learning for Computer Vision

## Publications
• Graph Temporal Attention Self-Supervised Network for Real-Time Anomaly Detection in Industrial IoT Systems (IEEE Access, 2026)
• LGA-SpecLiteNet: A Lightweight Multi-Branch Network with Adaptive Fusion for Deepfake Face Detection (ongoing)

## Featured Projects
- Deepfake Face Detection
- Graph Neural Networks
- Hospital Appointment System
- Android Time Scheduler
- Image Editor
